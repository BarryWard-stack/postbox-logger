...
### Issue Identified
**"Fake Transparency" Detected:** Uploaded icons were JPEGs with `.png` extension, containing baked-in checkered background pixels rather than true alpha channel.

**Root Cause:** Recraft AI generator exported transparency as flattened checkerboard pattern (common artifact in AI image tools).

**Impact if Unresolved:** Gray/white checkerboard grid would appear on user home screens, breaking PWA maskable icon functionality.

### Fix Applied (Authorized by Gemini - Option A)
1. Converted JPEG→PNG using ImageMagick
2. Applied `-fuzz 15% -transparent white` to strip checkered pixels
3. Replaced with true alpha channel: `Type: TrueColorAlpha` confirmed
4. Generated all 10 PWA sizes from clean 2000x2000 masters
5. **This conversion is now mandatory for all future Recraft exports**
